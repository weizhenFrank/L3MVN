...
from constants import coco_categories, hm3d_category
...
class ObjectGoal_Env(habitat.RLEnv):
    ...
    def reset(self):
        ...
        self.info['goal_name'] = hm3d_category[self.goal_idx]
    ...
    def get_llm_distance(self, target_point_map, frontier_loc_g):
        ...
        if tpm > 1:
            curr_loc = self.sim_continuous_to_sim_map(self.get_sim_location())
            frontier_loc_g[0][0] = frontier_loc_g[0][0] - target_point_map.shape[0]/2 + curr_loc[0]
            frontier_loc_g[1][0] = frontier_loc_g[1][0] - target_point_map.shape[1]/2 + curr_loc[1]
            (aa, bb) = self.gt_planner.traversible.shape
            if frontier_loc_g[0][0] > aa-1:
                frontier_loc_g[0][0] = aa-1
            if frontier_loc_g[1][0] > bb-1:
                frontier_loc_g[1][0] = bb -1
            ...
            for lay in range(tpm):
                ...
                curr_loc = self.sim_continuous_to_sim_map(self.get_sim_location())
                frontier_loc[0][0] = frontier_loc[0][0] - target_point_map.shape[0]/2 + curr_loc[0]
                frontier_loc[1][0] = frontier_loc[1][0] - target_point_map.shape[1]/2 + curr_loc[1]
                (aa, bb) = self.gt_planner.traversible.shape
                if frontier_loc[0][0] > aa-1:
                    frontier_loc[0][0] = aa-1
                if frontier_loc[1][0] > bb-1:
                    frontier_loc[1][0] = bb -1
